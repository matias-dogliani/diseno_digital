...
def arrayFixedInt(totalWidth, fractWidth, N, signedMode='S', roundMode='trunc', saturateMode='saturate'):
  '''Create a Numpy array of length N with DeFixedInt instances of
  representation S(totalWidth, fractWidth).
  If value is specified the instances are set with the specified value.

  @type   totalWidth    : integer
  @param  totalWidth    : used total bit width
...
class DeFixedInt(object):
  ...
  def __init__(self, totalWidth=0, fractWidth=15, signedMode='S', roundMode='trunc', saturateMode='saturate'):

    '''    
    @type   totalWidth    : unsigned integer number
    @param  totalWidth    : Number of bits used to store the 
    ...
    if(totalWidth - fractWidth < 0):
        if(abs(totalWidth - fractWidth)>=fractWidth):
    ...
    self.__intWidth = totalWidth - fractWidth-1  # Remove Sign Bit
